...
function init($aFormValue){
    ...
    $objResponse->addAssign("divCopyright","innerHTML", (new Common)->generateCopyright($skin));
...
function processAccountData($aFormValues)
{
	...
	if (!$bError)
	{
        ...
        $row = (new astercrm)->getRecordByField("username",$aFormValues['username'],"astercrm_account");
		...
		if ($row['id'] != '' ){
			if ($row['password'] == $aFormValues['password'])
			{
				...
				if ($row['usertype'] == 'groupadmin' || is_array($_SESSION['curuser']['privileges']['systemstatus'])  || is_array($_SESSION['curuser']['privileges']['import'])){
                    ...
                    $groupList = (new astercrm)->getGroupMemberListByID($row['groupid']);
				...
				if($_SESSION['curuser']['groupid'] > 0){
					$sql = "SELECT id
                              ...
                              FROM campaign 
                             WHERE queuename != '' 
                               AND groupid='".$_SESSION['curuser']['groupid']."' 
                               AND enable= 1 
