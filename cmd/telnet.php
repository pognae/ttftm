	...
	function Connect($server,$user,$pass) {
		...
		if (strlen($ip)) {
			if ($this->fp=fsockopen($ip,5038)) {
				...
				fputs($this->fp,"\r\nAction: Login\r\n");
				fputs($this->fp,"Username: $user\r\n");
				fputs($this->fp,"Secret: $pass\r\n\r\n");
				$this->Sleep();
				if ($this->use_usleep) usleep($this->loginsleeptime);
				else sleep(1);
				$this->GetResponse($r);
				if (stristr($r, 'Response: Error')) {
					$rv=3;
					$this->Disconnect();
				}
			} else $rv=1;
